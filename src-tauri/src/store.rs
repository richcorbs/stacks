    ...
    let store = kanban::with_connection(|connection| {
        ...
        transaction.execute(
            ...
            params![input.id, input.name.trim(), input.path.trim(), source, non_empty(input.start_work_command), spaces, slug,
                ...
                normalize_delivery_workflow(&input.delivery_workflow), non_empty(input.deployment_command), target, input.supports_feature_environments as i64,
...
fn update_project_configuration_validated(
    ...
    let store = kanban::with_connection(|connection| {
        ...
        let (previous_source, previous_board_id, current_revision, previous_mapping_revision, previous_workflow) = connection.query_row(
            "SELECT COALESCE(kanban_source, 'local'), superthread_board_id, config_revision, superthread_mapping_revision, delivery_workflow FROM projects WHERE id=?1", [&input.id], |row| Ok((row.get::<_, String>(0)?, row.get::<_, Option<String>>(1)?, row.get::<_, i64>(2)?, row.get::<_, i64>(3)?, row.get::<_, String>(4)?)),
    ...
    let projects = project_statement
        .query_map([], |row| {
            Ok(Project {
                ...
                delivery_workflow_locked: row.get::<_, i64>(31)? != 0,
                notes: row.get(3)?,
