...
fn require_structural_capability(
    connection: &Connection,
    id: &str,
...
pub fn kanban_close_card(id: String, expected_revision: i64) -> Result<KanbanCard, String> {
    with_connection(|connection| {
        let transaction = connection
            .transaction_with_behavior(TransactionBehavior::Immediate)
            .map_err(db_error)?;
        let (status, revision, finalized): (CardStatus, i64, bool) = transaction
            .query_row(
        ...
        apply_workflow_transition(
            ...
            Some(expected_revision),
            "close",
            Some("Closed without delivery; work preserved"),
        )?;
...
fn approve_and_commit(
    ...
    let transaction = connection
        .transaction_with_behavior(TransactionBehavior::Immediate)
        .map_err(db_error)?;
    let (status, card_revision): (CardStatus, i64) = transaction
        .query_row(
    ...
    if status != CardStatus::AgentWorking {
        require_structural_capability(&transaction, id, action)?;
    }
...
fn merge_card(
    ...
    let _ = status;
    require_structural_capability(&transaction, id, WorkflowAction::MergeLocal)?;
...
pub async fn kanban_cleanup_environment(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        let result = with_connection(|connection| {
            ...
            let (status, completion_outcome, card_revision, delivery_stage): (CardStatus, Option<CompletionOutcome>, i64, Option<String>) = transaction.query_row("SELECT status, completion_outcome, workflow_revision, delivery_operation_stage FROM kanban_cards WHERE id=?1", [&id], |row| Ok((row.get(0)?, row.get(1)?, row.get(2)?, row.get(3)?))).map_err(db_error)?;
            if card_revision != expected_workflow_revision { return Err("Card changed; reload before cleanup".to_string()); }
            let _ = status;
            require_structural_capability(&transaction, &id, WorkflowAction::Cleanup)?;
            ...
            if completion_outcome == Some(CompletionOutcome::Merged) && current_tip != recorded_tip { return Err("The source branch has new commits since merge; merge again before cleanup".to_string()); }
            ...
            if completion_outcome == Some(CompletionOutcome::Merged) && !rewritten_pr_evidence && !Command::new("git").args(["-C", &target_path, "merge-base", "--is-ancestor", &current_tip, "HEAD"]).status().map_err(|error| error.to_string())?.success() {
            ...
            if completion_outcome == Some(CompletionOutcome::Merged) {
            ...
            transaction.execute("UPDATE kanban_cards SET updated_at=?1 WHERE id=?2 AND workflow_revision=?3", params![unix_timestamp(), id, expected_workflow_revision]).map_err(db_error)?;
            transaction.execute("INSERT INTO card_events (card_id, created_at, actor, event_type, outcome, summary) VALUES (?1, ?2, 'user', 'cleanup', 'success', ?3)", params![id, unix_timestamp(), if completion_outcome == Some(CompletionOutcome::Closed) { "Removed source worktree and retained branch" } else { "Removed source worktree and branch" }]).map_err(db_error)?;
            transaction.commit().map_err(db_error)?;
            get_card(connection, &id)?.ok_or_else(|| "Kanban card was not found".to_string())
        });
...
fn enrich_capabilities(connection: &Connection, cards: &mut [KanbanCard]) -> Result<(), String> {
    for card in cards {
        card.capabilities = workflow::capabilities(&workflow_context_for_card(connection, card)?);
    }
    Ok(())
}
