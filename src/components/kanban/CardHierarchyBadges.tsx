...
export function CardHierarchyBadges({ card, onNavigateParent }: { card: KanbanCardSummary; onNavigateParent: (parentId: string) => void }) {
  ...
  const parentLabel = card.parent ? `Parent #${card.parent.external_id}: ${card.parent.title}` : '';
  const parentButton = card.parent && <button
    className={`kanbanHierarchyBadge parent${card.child_count > 0 ? ' combined' : ''}`}
    type="button"
    title={card.child_count > 0 ? `${parentLabel}; ${childCountLabel(card.child_count)}` : parentLabel}
    aria-label={card.child_count > 0 ? `${parentLabel}; ${childCountLabel(card.child_count)}` : parentLabel}
  ...
  return <span className="kanbanHierarchyBadge children" aria-label={childCountLabel(card.child_count)}>
    {card.child_count}
